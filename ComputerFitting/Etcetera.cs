...
namespace ComputerFitting
{
    ...
    public partial class Etcetera : Form
    {
        public Fitting fit;
        public List<Etc> data = new List<Etc>();
        public Etcetera()
        {
            InitializeComponent();
        }
        ...
        private void Etcetera_Load(object sender, EventArgs e)
        {
            LoadEtcetera();
            RefreshTable();
        }

        private void LoadEtcetera()
        {
            string filename = "C:\\DB\\Etc.data";
            using (var file = File.OpenRead(filename))
            {
                ...
                try
                {
                    data = (List<Etc>)reader.Deserialize(file); // Reads the entire list.
                }
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                foreach (String a in data[i].notCompatible)
                {
                    temp.SubItems.Add(a + ";");
                }
        ...
        private void Etcetera_FormClosing(object sender, FormClosingEventArgs e)
        {
            SaveEtcetera();
        }
